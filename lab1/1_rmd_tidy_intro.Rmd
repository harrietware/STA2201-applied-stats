...
```{r}
dm <- read_table(here("lab1","CAN_Mx_1x1.txt"), skip = 2)
head(dm)
```
...
dm_to_plot <- dm %>% 
  ...
  pivot_longer(Female:m_f_ratio, "sex", values_to = "mx") %>% 
  filter(sex=="m_f_ratio")
dm_to_plot
```
```{r}
p2 <- ggplot(dm_to_plot, aes(Year, mx, color = Age, group = Age)) +
  geom_line() + 
  ylab("Ratio of male to female mortality rate")+
  ggtitle("Ratio of male to female mortality rates for Canadians, 1921-2011")
p2+theme_bw()
```
